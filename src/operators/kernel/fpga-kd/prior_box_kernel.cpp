...
void PriorBoxKernel<FPGA, float>::Compute(const PriorBoxParam<FPGA> &param) {
  Tensor *output_boxes = param.OutputBoxes();
  Tensor *output_variances = param.OutputVariances();
  output_boxes->set_data_aligned(false);
  output_variances->set_data_aligned(false);

  if (param.get_cache_boxes() == nullptr) {
    PriorBoxParam<FPGA>& p = const_cast<PriorBoxParam<FPGA>&>(param);

    p.make_cache(output_boxes->numel() * sizeof(half), output_variances->numel() * sizeof(half));
