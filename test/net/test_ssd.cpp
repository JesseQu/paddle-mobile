...
void run_entire(paddle_mobile::PaddleMobile<paddle_mobile::FPGA>& paddle_mobile) {
  ...
  int OpNum = paddle_mobile.OpNum();

  std::cout << OpNum << " nodes need to be processed! " << std::endl;

  size_t sum = 0;
  size_t sum_max = 0;
  sum = fpga::fpga_diagnose_memory(0);
  sum_max = fpga::fpga_get_memory_size_max();
  std::cout << "================================================================" << std::endl;
  std::cout << "Total memories got remained: base - " << sum << ", max - " << sum_max << std::endl;
  std::cout << "================================================================" << std::endl;
  ...
  /*
    for (int i = 0; i < OpNum; i++) {
      ...
      for (int n = 0;n < out_num; n++) {
        ...
        // }
        std::cout << "dumping No. " << i << " - " << n << std::endl;
...
int main() {
  ...
  if (paddle_mobile.Load(model, params, true)) {
    ...
    std::cout << "predict will begin :" << std::endl;
